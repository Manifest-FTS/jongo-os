...
export default async function ClientSettingsPage({ params }: Params) {
  ...
  return (
    <div className="page-stack">
      ...
      {isAdmin ? (
        <article className="card">
          <h3 className="card-title">Platform Project Mapping</h3>
          <p className="card-muted" style={{ marginBottom: "1rem" }}>
            Link one or more platform projects to this client workspace. Project links are explicit and never auto-renamed.
          </p>
          {linkedProjects.length === 0 ? (
            <div className="diagnostic-banner" style={{ marginBottom: "1rem" }}>
              <strong>No linked platform project.</strong> Link at least one project so this client's apps map cleanly in Jongo.
            </div>
          ) : null}
          {client.dbId ? (
        ...
        <article className="card">
          ...
          <p className="card-muted">Platform ownership mapping is restricted to admin users.</p>
        </article>
      )}
    </div>
  );
