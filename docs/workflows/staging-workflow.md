...
- create missing Coolify project staging environments in controlled batches via `npm run ops:remediate-staging-environments -- <app-slug...>` and apply with `npm run ops:remediate-staging-environments:apply -- <app-slug...>` when capability note indicates `project_only_has_production_environment`
	- requires a Coolify API token with write permission for project environment mutation (`POST /api/v1/projects/{uuid}/environments`)
